# Thai QR Code Tool with JSON Syntax Highlighting, Summary, and Validation
...
class JsonHighlighter(tk.Text):
    def __init__(self, parent, **kwargs):
        super().__init__(parent, wrap="word", **kwargs)
        self.configure(font=("Consolas", 10))
        self.tag_configure("key", foreground="#d73a49")
        self.tag_configure("string", foreground="#032f62")
        self.tag_configure("number", foreground="#005cc5")
        self.tag_configure("bool", foreground="#e36209")
        self.tag_configure("null", foreground="#6f42c1")
    ...
    def highlight(self, content):
        ...
        for idx, line in enumerate(lines):
            ...
            self._highlight_line(start, end)

    def _highlight_line(self, start, end):
        text = self.get(start, end)
        for match in re.finditer(r'(\s*)(".*?")(\s*:\s*)(.*)', text):
            groups = match.groups()
            offset = len(groups[0])
            self.tag_add("key", f"{start}+{offset}c", f"{start}+{offset+len(groups[1])}c")

        for match in re.finditer(r':\s*(".*?")', text):
            self.tag_add("string", f"{start}+{match.start(1)}c", f"{start}+{match.end(1)}c")
        for match in re.finditer(r':\s*(-?\d+(\.\d+)?)', text):
            self.tag_add("number", f"{start}+{match.start(1)}c", f"{start}+{match.end(1)}c")
        for match in re.finditer(r':\s*(true|false)', text):
            self.tag_add("bool", f"{start}+{match.start(1)}c", f"{start}+{match.end(1)}c")
        for match in re.finditer(r':\s*(null)', text):
            self.tag_add("null", f"{start}+{match.start(1)}c", f"{start}+{match.end(1)}c")
...
class ThaiQRGUI:
    def __init__(self, root):
        ...
        self.decode_tab = tk.Frame(self.tab)
        self.generate_tab = tk.Frame(self.tab)
        self.modify_tab = tk.Frame(self.tab)
        self.validate_tab = tk.Frame(self.tab)
        ...
        self.output = JsonHighlighter(self.decode_tab, height=25, width=100)
        self.output.pack()

        # Generate Tab
        ttk.Label(self.generate_tab, text="Paste JSON or QR Payload to Encode:").pack()
        self.json_input = JsonHighlighter(self.generate_tab, height=20, width=100)
        ...
        self.modify_json = JsonHighlighter(self.modify_tab, height=20, width=100)
        ...
        self.validate_input = JsonHighlighter(self.validate_tab, height=20, width=100)
    ...
    def decode_qr(self):
        try:
            ...
            self.output.highlight(json_text)
            self.modify_json.highlight(json_text)
